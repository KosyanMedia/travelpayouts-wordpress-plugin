...
class TPHotelShortcodeView //extends TPShortcodeView
{
    ...
    {
        add_action('wp', [&$this, 'redirectPlugins']);
    }
    ...
    public function redirectPlugins()
    {
        ...
        if ($redirect) {
            ...
            if (TPRequest::get('search_hotel')) {
                $queryValue = urldecode(TPRequest::get('search_hotel'));
                $whiteLabel = $this->getWhiteLabel();
                $white_label = $whiteLabel['white_label'];
                $white_label = "{$white_label}{$queryValue}";
     ...
     * @return bool
     */
    public function isEmptyPrice($rows = [])
    {
        //$i = 0;
        ...
        if ($off_title !== 'true') {
            if (empty($title)) {
                if (isset(TPPlugin::$options['shortcodes_hotels'][$shortcode]['title'][TPLang::getLang()])) {
                    ...
                    $title = TPPlugin::$options['shortcodes_hotels'][$shortcode]['title'][TPLang::getDefaultLang()];
                }
            }
            if (!empty($title)) {
            ...
            return '<' . TPPlugin::$options['shortcodes_hotels'][$shortcode]['tag'] . ' class="TP-TitleTables">' . $title . '</' . TPPlugin::$options['shortcodes_hotels'][$shortcode]['tag'] . '>';
        }
        return '';
    }
}
