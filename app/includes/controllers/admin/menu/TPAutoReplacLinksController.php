...
use app\includes\models\admin\menu\TPAutoReplacLinksModel;
use app\includes\models\admin\menu\TPAutoReplacLinksOptionModel;
use app\includes\TPPlugin;
use core\controllers\TPOAdminMenuController;
use core\TPRequest;

class TPAutoReplacLinksController extends TPOAdminMenuController
{
    public $model;
    public $modelOption;
    public $data;
    ...
    public function __construct()
    {
        ...
        $this->model = new TPAutoReplacLinksModel();
        $this->modelOption = new TPAutoReplacLinksOptionModel();

        add_action('save_post', [&$this, 'autoReplacLinksSavePost'], 10, 3);

        add_action('add_meta_boxes', [&$this, 'tp_add_custom_box']);
        add_action('wp_footer', [&$this, 'renderProgressbar']);
        add_action('admin_footer', [&$this, 'renderProgressbar']);

        add_action('admin_footer-edit.php', [&$this, 'renderButton']);
        add_filter('post_row_actions', [&$this, 'renderLinkPost'], 10, 2);
        add_filter('page_row_actions', [&$this, 'renderLinkPost'], 10, 2);
        add_action('wp_ajax_auto_replace_link_post_by_id', [&$this, 'TPAutoReplaceLinkPostById']);
        add_action('wp_ajax_auto_replace_link_post_check_by_id', [&$this, 'TPAutoReplaceLinkPostCheckById']);
        add_action('wp_ajax_replace_all', [&$this, 'replaceAll']);
        add_action('wp_ajax_replace_insert_post', [&$this, 'replaceInsertPost']);
        add_action('wp_ajax_tp_export_links', [&$this, 'tpExportLinks']);
    }


    public function TPAutoReplaceLinkPostCheckById()
    {
        if (TPRequest::post('id')) {
    ...
    {
        if (TPRequest::post('id')) {
            $post = get_post(TPRequest::post('id'), ARRAY_A);
    ...
    {
        if (TPRequest::post('id')) {
            $id = TPRequest::post('id');
            $dataAutoReplacLinks = $this->model->get_dataByArrayId($id);
            if ($dataAutoReplacLinks == false) {
                return false;
            }
     ...
     * @return array|int
     */
    public function getReplaceLimits()
    {//$count_anchor
        $limit = TPPlugin::$options['auto_repl_link']['limit'];
        if ($limit == 0) {
            return -1;
        }
        return (int)$limit;
     ...
     * @return array
     */
    public function getReplaceLimitRecursive($limit, $count_anchor, $limit_array = [])
    {
        if (TpPluginHelper::count($limit_array) === 0) {
            $limit_array = array_pad([], $count_anchor, 0);
        }
    ...
    {
        //error_log($matches);
        if (strpos($matches, '<img') !== false) {
            return stripslashes($matches);
        }
        if (strpos($matches, '<h1') !== false) {
            //Если в тексте нет заголовка
            if (isset(TPPlugin::$options['auto_repl_link']['not_title'])) {
                //Не вставлять ссылки во все заголовки
                //error_log("Заменна ссылок replace == 1 h1");
    ...
    public function tp_add_custom_box()
    {
        ...
        foreach ($screens as $screen) {
            add_meta_box(
                ...
                [&$this, 'tp_add_custom_box_callback'],
                $screen,
                'side',
                'high'
            );
    ...
    public function tp_add_custom_box_callback($post)
    {
        ...
        if ($dataAutoReplacLinks !== false) {
            if (get_post_meta($post->ID, 'tp_auto_replac_link', true)) {
                ...
                //error_log("option");
                if (isset(TPPlugin::$options['auto_repl_link']['tp_auto_replac_link'])) {
    ...
    public function autoReplacLinksSavePost($post_id, $post, $update)
    {
        ...
        $nonceNameValue = TPRequest::post('tp_auto_replac_link_noncename');


        if (!$nonceNameValue) {
            return $post_id;
        }
        if (!wp_verify_nonce($nonceNameValue, TPOPlUGIN_NAME)) {
            return $post_id;
        }
        ...
        $tp_auto_replac_link = sanitize_text_field(TPRequest::post('tp_auto_replac_link'));
